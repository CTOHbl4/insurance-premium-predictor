...
df['YEAR'] = df['INSR_BEGIN'].apply(lambda x: int(x.split('-')[2]))

thresh_year = 13
df_train = df[df['YEAR'] <= thresh_year].copy()
df_test = df[df['YEAR'] > thresh_year].copy()
...
df_test_processed, _, _ = predictor.processor.transform(df_test, train=False)
actuals = df_test_processed['PREMIUM'].values
