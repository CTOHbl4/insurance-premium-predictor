...
from ml.eda.make_map import get_make_map
...
class InsuranceDataProcessor:
    ...
    def __init__(
        ...
        posthoc_dunn: float = 0.05,
        mann_whitney_thresh: float = 0.05,
        apriori_min_support: float = 0.01,
        apriori_min_confidence: float = 0.99
    ):
        """
    ...
    def _apply_filter(self, df: pd.DataFrame, filter_config: dict, filter_key: str, train: bool = False) -> Tuple[pd.DataFrame, int]:
        ...
        if filter_key.startswith('MIX_'):
            ...
            if filter_key == 'PREMIUM' and 'PREMIUM' not in df.columns.to_list():
                return df, 0

            if train:
                df = df[df[filter_key] < rule]
            else:
                df.loc[df[filter_key] > rule, filter_key] = rule
            ...
            if train:
                ...
                for threshold, vehicle_type in rule:
                    df.loc[(df['TYPE_VEHICLE'] == vehicle_type) & (df[filter_key] > threshold), filter_key] = threshold
    ...
    def _parse_rule_item(self, item: str) -> Dict[str, Any]:
        ...
        return {col: int(val) if col in ('INSR_TYPE', 'SEX') else val}
    ...
    def _handle_nans(self, df: pd.DataFrame, train: bool = False) -> Tuple[pd.DataFrame, Dict]:
        ...
        df = df.sort_values(['START_MNTH', 'OBJECT_ID', 'DURATION'])
    ...
    def filter_outliers(self, df: pd.DataFrame, train: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame, Dict]:
        ...
        dfs = {'ELSE': df_else, 'INSR_ZERO': df_zero}
        for config_name in ('ELSE', 'INSR_ZERO'):
            quality_metrics[f'{config_name}_insured_records'] = len(dfs[config_name])
            for filter_key in filter_config[config_name].keys():
                dfs[config_name], removed = self._apply_filter(dfs[config_name], filter_config[config_name], filter_key, train)
                if removed > 0:
                    quality_metrics['removed_by_filter'][f'{config_name}_{filter_key}'] = removed
            quality_metrics[f'final_{config_name}_records'] = len(dfs[config_name])
        df_else, df_zero = dfs['ELSE'], dfs['INSR_ZERO']
    ...
    def transform(self, df: pd.DataFrame, train: bool = True) -> Tuple[pd.DataFrame, pd.DataFrame, Dict]:
        ...
        df_zero, df_else, metrics_outliers = self.filter_outliers(df, train)
    ...
    def fit(self, df: pd.DataFrame) -> None:
        self._save_config(self.CONFIG_NAMES['MAKE'], get_make_map())
        ...
        for df_part, insr_key in [(df[df['INSURED_VALUE'] == 0], 'INSR_ZERO'),
            ...
            for col in ['USAGE', 'TYPE_VEHICLE', 'MAKE']:
